...
export const ecologicaleResourcesMixin = {
  ...
  mounted() {
    ...
    getScaleData({
      ...
      this.chartArr.sideItem2_0.result = `根据${data.yAxis.year}年调查数据，${this.cityName}森林覆盖率<span class="light">${this.chartArr.sideItem2_0.yAxis[i]}</span>%，在全省11地市中排第<span class="light">${i + 1}</span>位，<span class="light">${aaa}</span>全省平均水平。`
    })
    // 1、全省各地市活立木蓄积量占比
    getScaleDataWithName({
      area: '浙江省',
      ...
      this.chartArr.sideItem2_1.unit = data.unit
    ...
    getScaleDataWithName({
      ...
      this.chartArr.sideItem2_2.unit = data.unit
    ...
    getScaleData({
      ...
      let aaa = this.returnDiff(this.returnArrAvg(data.yAxis.list), data.yAxis.list[i]) >= 0 ? '高于' : '低于'
      this.chartArr.sideItem2_3.result = `根据${data.yAxis.year}年调查数据，${this.cityName}单位面积森林蓄积量为<span class="light">${this.chartArr.sideItem2_3.yAxis[i]}</span>（立方米/公顷），在全省11地市中排第<span class="light">${i + 1}</span>位，<span class="light">${aaa}</span>全省平均水平。`
    })
    // 4、获取市各县、市森林覆盖率并绘制图表
    getScaleData({
      area: this.cityName,
      ...
      let aaa = this.returnRepeatNumArr(this.chartArr.sideItem2_4.xAxis, this.chartArr.sideItem2_4.yAxis, this.chartArr.sideItem2_4.yAxis[this.chartArr.sideItem2_4.yAxis.length - 1], 'max')
      let bbb = this.returnRepeatNumArr(this.chartArr.sideItem2_4.xAxis, this.chartArr.sideItem2_4.yAxis, this.chartArr.sideItem2_4.yAxis[0], 'min')

      this.chartArr.sideItem2_4.result = `${data.yAxis.year}年${this.cityName}下辖各县市中，${bbb.join()}森林覆盖率最高，达到<span class="light">${this.chartArr.sideItem2_4.yAxis[0]}</span>%，${aaa.join()}最低，为<span class="light">${this.returnArrLast(this.chartArr.sideItem2_4.yAxis)}</span>%。`
    })
    ...
    getScaleData({
      ...
      let aaa = this.returnRepeatNumArr(this.chartArr.sideItem2_5.xAxis, this.chartArr.sideItem2_5.yAxis, this.chartArr.sideItem2_5.yAxis[this.chartArr.sideItem2_5.yAxis.length - 1], 'max')
      let bbb = this.returnRepeatNumArr(this.chartArr.sideItem2_5.xAxis, this.chartArr.sideItem2_5.yAxis, this.chartArr.sideItem2_5.yAxis[0], 'min')
      this.chartArr.sideItem2_5.result = `${data.yAxis.year}年${this.cityName}下辖各县市中，${bbb.join()}的单位面积森林蓄积量最高，达到<span class="light">${this.chartArr.sideItem2_5.yAxis[0]}</span>立方米/公顷，${aaa.join()}最低，为<span class="light">${this.returnArrLast(this.chartArr.sideItem2_5.yAxis)}</span>立方米/公顷。`
    })
  },
  methods: {
  }
