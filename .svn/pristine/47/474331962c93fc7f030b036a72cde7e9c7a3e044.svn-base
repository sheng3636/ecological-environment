...
export const contactMixin = {
  ...
  mounted() {
    ...
    this.chartArr.sideItem5_6.title = `2018年基于联通手机信令数据的${this.cityName}与长三角各区域黄金周人口联系情况`
    this.chartArr.sideItem5_7.title = `2018年基于联通手机信令数据的${this.cityName}与长三角各区域普通周人口联系情况`
    ...
    apiGet(
      ...
      this.chartArr.sideItem5_3.result = `2018年，${this.cityName}与<span class="light">${bb.join()}</span>的人口联系最为密切。`
    })
    ...
    apiGet(
      ...
      apiGet(
        ...
        this.chartArr.sideItem5_6.result = `2018年，在长三角范围内，${this.cityName}的人口联系以省内为主，在省内的人口相互联系次数占整个长三角区域的人口相互联系总次数的<span class="light">${parcent1}</span>%以上；与上海市的相互联系次数占比在普通周为<span class="light">${parcent2}</span>%； 除浙江省外，${this.cityName}与<span class="light">${areas}</span>的人口联系最为密切。`
      })
    })
