...
export const environmentalQualityMixin = {
  ...
  mounted() {
    ...
    // 1、获取某年某市管辖各县级城市空气质量达到优良天数比例并绘制图表
    getScaleData({
      area: this.cityName,
      zb: 'AQI优良率'
    }).then(res => {
      let data = res.data
      this.chartArr.sideItem1_1.title = `${data.yAxis.year}年${this.cityName}管辖各县级城市空气质量达到优良天数比例`
      ...
      let aaa = this.returnRepeatNumArr(this.chartArr.sideItem1_1.xAxis, this.chartArr.sideItem1_1.yAxis, this.chartArr.sideItem1_1.yAxis[this.chartArr.sideItem1_1.yAxis.length - 1], 'max')
      let bbb = this.returnRepeatNumArr(this.chartArr.sideItem1_1.xAxis, this.chartArr.sideItem1_1.yAxis, this.chartArr.sideItem1_1.yAxis[0], 'min')
      this.chartArr.sideItem1_1.result = `${data.yAxis.year}年，${this.cityName}管辖的各县级城市中，${aaa.join()}空气质量达到优良天数比例最低，达到<span class="light">${this.returnArrLast(this.chartArr.sideItem1_1.yAxis)}</span>%；${bbb.join()}最高，为<span class="light">${this.chartArr.sideItem1_1.yAxis[0]}</span>%。`
    })
    ...
    getStructuralData({
      ...
      let data = res.data[0]
      this.chartArr.sideItem1_2.title = `${this.returnArrFirst(data.xAxis)}年-${this.returnArrLast(data.xAxis)}年${this.countyName}PM2.5年平均浓度（µg/m³）变化`
      ...
      let aaa = this.chartArr.sideItem1_2.yAxis[this.chartArr.sideItem1_2.yAxis.length - 1]
      let qualified = this.returnDiff(aaa, 35) > 0 ? '达到' : '不达到'
      this.chartArr.sideItem1_2.result = `${this.returnArrLast(this.chartArr.sideItem1_2.xAxis)}年，${this.countyName}PM2.5浓度年均达到<span class="light">${aaa}</span>µg/m³，<span class="light">${qualified}</span>国家空气质量二级标准要求（35µg/m³）。`
    })

    // 3、获取某市管辖各县级城市PM2.5年均浓度（µg/m³）对比并绘制图表
    getScaleData({
      area: this.cityName,
      zb: 'PM2.5浓度'
    }).then(res => {
    ...
    sectionNum({
      ...
      let data = res.data
      this.chartArr.sideItem1_4.title = `${data.yAxis.year}${this.cityName}各县（市、区）III类以上水质断面数量`
      this.chartArr.sideItem1_4.dataSource = '省生态环境厅'
      this.chartArr.sideItem1_4.xAxis = data.xAxis
      this.chartArr.sideItem1_4.yAxis1 = data.yAxis.threeList
      this.chartArr.sideItem1_4.yAxis2 = data.yAxis.numList
