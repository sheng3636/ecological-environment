...
export const societyMixin = {
  ...
  mounted() {
    ...
    apiGet(
      ...
      this.chartArr.sideItem3_0.title = `${aaa}年-${ccc}年${this.cityName}城镇、 农村常住居民人均可支配收入变化`
    ...
    axiosAll([this.getCitySociData3(), this.getCitySociData4()]).then(res => {
      let data1 = res[0].data
      let year = data1.yAxis[0].year
      let list = data1.yAxis[0].list.reverse()
      ...
      this.chartArr.sideItem3_2.xAxis = data1.xAxis.reverse()
      ...
      let avgStr = (diff == 0) ? `与全省平均水平<span class="light">持平</span>` : diff < 0 ? `<span class="light">高于</span>全省平均水平` : `<span class="light">低于</span>全省平均水平`
      ...
      if (rank.changeVal === undefined) {
        ...
        rankStr = rank.changeVal == 0 ? `排名较上年<span class="light">维持不变</span>` : rank.changeVal < 0 ? `排名较上年上升<span class="light">${Math.abs(rank.changeVal)}</span>位` : `排名较上年下降<span class="light">${rank.changeVal}</span>位`
      }

      this.chartArr.sideItem3_2.result = `${year}年，${this.cityName}城乡居民收入比${avgStr}，在全省排第<span class="light">${index + 1}</span>位，${rankStr}。`
    })
