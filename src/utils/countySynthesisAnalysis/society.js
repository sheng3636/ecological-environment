...
export const societyMixin = {
  ...
  mounted() {
    ...
    axiosAll([this.getCountySociData7(), this.getCountySociData8()]).then(res => {
      let data1 = res[0].data
      let tableList =  data1.Table.reverse()
      for(let i = 1;i <= tableList.length;i++){
        tableList[i-1].rank = i
      }
      this.chartArr.sideItem3_4.title = `2018年全省各县（市、区）城乡居民收入比对比`
      this.chartArr.sideItem3_4.xAxis = data1.xData.reverse()
      this.chartArr.sideItem3_4.chartArr[0].yAxis = data1.yData.reverse()
      this.chartArr.sideItem3_4.list = data1.yData
      this.chartArr.sideItem3_4.list = tableList
    ...
    axiosAll([this.getCountySociData9(), this.getCountySociData10()]).then(res => {
      ...
      this.chartArr.sideItem3_5.title = `${aaa}年-${ccc}年${this.countyName}常住城镇化率变化`
