...
export const synthesisMixin = {
  ...
  mounted() {
    ...
    apiGet(
      ...
      this.chartArr.sideItem6_0.result = `2018年${this.countyName}的市内优势指标有<span class="light">${ccc.join(',')}</span>， 劣势指标有<span class="light">${ddd.join(',')}</span>。`
    })
    ...
    apiGet(
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        this.chartArr.sideItem6_1.chartArr[0].seriesData.push(data[i].endRank)
      }

      this.chartArr.sideItem6_1.title = `2018年${this.countyName}主要指标省内排名`
      this.chartArr.sideItem6_1.list = data
      ...
      this.chartArr.sideItem6_1.result = `2018年${this.countyName}的省内优势指标有<span class="light">${ccc.join(',')}</span>， 劣势指标有<span class="light">${ddd.join(',')}</span>。`
    })
    ...
    apiGet(
      ...
      let aaa = this.chartArr.sideItem6_2.chartArr[0].yAxis
      ...
      let eee = []
      for (let i = 0; i < bbb.length; i++) {
        ddd.push(this.chartArr.sideItem6_2.xAxis[bbb[i]])
      }
      for (let i = 0; i < ccc.length; i++) {
        eee.push(this.chartArr.sideItem6_2.xAxis[ccc[i]])
      }
      ...
      this.chartArr.sideItem6_2.result = `2018年，${this.countyName}进度良好的指标${fff}， 进度滞后的指标${ggg}。`
    })
  },
  methods: {}
}