...
export const serviceIndustryMixin = {
  ...
  mounted() {
    ...
    apiGet(
      ...
      this.chartArr.sideItem3_0.title = `2018年${this.cityName}服务业分行业增加值规模`
      if (res.code === '0') {
        let data = res.data
        ...
        this.chartArr.sideItem3_0.result = `2018年${this.cityName}服务业规模前5产业分别为<span class="light">${data.xData.slice(0,5).join('、')}</span>。`
      } else {
    ...
    apiGet(
      ...
      this.chartArr.sideItem3_1.title = `2018年${this.cityName}服务业分行业占比变化`
      if (res.code === '0') {
        let data = res.data
        ...
        this.chartArr.sideItem3_1.result = `2018年${this.cityName}近五年占比提升排名分别为<span class="light">${data.xData.join('、')}</span>。`
      } else {
    ...
    axiosAll([this.serviceIndustryIAll1(), this.serviceIndustryIAll2()]).then(res => {
      this.chartArr.sideItem3_3.title = `2018年${this.cityName}服务业分行业工商注册企业数量`
      this.chartArr.sideItem3_4.title = `2018年${this.cityName}服务业分行业工商注册企业新增数量`
      let aaa = ''
      let bbb = ''
      if (res[0].code === '0') {
        ...
        aaa = `2018年${this.cityName}共有服务业工商注册企业<span class="light">${sum}</span>万家，主要分布在<span class="light">${data0.xData.slice(0,5).join('、')}</span>等行业。`
      } else {
      ...
      if (res[1].code === '0') {
        ...
        let sum1 = 0
        for (var i = 0; i < data1.yData.length; i++) {
          sum1 += data1.yData[i]
        }
